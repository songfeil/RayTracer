...
Object * random_scene() {
  ...
  Material * mat = new metal(new imageTexture(tex_data, nx, ny), 0.0);

//  list[i++] = new Sphere(Eigen::Vector3d(0, 1, 0), 1.0, new dielectric(2.5));

  TriangleMesh * tm = new TriangleMesh("triangles.obj", mat);
  tm->translate(Eigen::Vector3d(0, 1, 0));
  list[i++] = tm;
  list[i++] = new Sphere(Eigen::Vector3d(-4, 1, 0), 1.0, new dielectric(2.5));
